...
namespace assignment.Controllers
{
    public class ProductController : Controller
    {
        ...
        public IActionResult Create(ProductCreateVM productCreateVM)
        {
            ...
            if (productCreateVM.Image != null)
            {
                var uniqFileName = Guid.NewGuid().ToString() + Path.GetExtension(productCreateVM.Image.FileName);
                ...
                imageURL = "/images/Product/" + uniqFileName;
            }
        ...
        public IActionResult Edit(ProductEditVM productEditVM)
        {
            ...
            if (productEditVM.Image != null)
            {
                ...
                {
                    var oldFilePath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", productInDb.ImageURL.TrimStart('/'));
                ...
                var uniqFileName = Guid.NewGuid().ToString() + Path.GetExtension(productEditVM.Image.FileName);
                var folderPath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "images", "Product");
                if (!Directory.Exists(folderPath))
                {
                    Directory.CreateDirectory(folderPath);
                ...
                using (var stream = new FileStream(filePath, FileMode.Create))
                {
                    productEditVM.Image.CopyTo(stream);
                }
                productInDb.ImageURL = "/images/Product/" + uniqFileName;
            }
